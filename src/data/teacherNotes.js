...
export const TEACHER_NOTES = [
  ...
  // ── 16: SectionLayers (2 slides) ──
  {
    ...
    slides: [
      {
        keyPoints: ["One layer = look (attention), then think (MLP)", "The class predicts how many more layers before the reveal", "Then watch the other 93 fly by"],
        narrative: "Let the first layer play slowly and narrate it: 'Every word looks at the other words. Then the thinking part asks questions — spelling, grammar, facts, meaning.' When it pauses, take the vote: 1 more? 3? 12? 95? 3,200? Reveal: today's big models use roughly 50–100 layers; our example does it 95 more times. Then press 'Run the other 95 layers' and let the questions evolve from 'Is this spelled right?' (early) to 'Which word fits best?' (late).",
        definitions: [{ term: "Layer", def: "One complete round of Attention + MLP — models stack many of these in sequence" }],
        discussion: ["Why might it take so many rounds to pick one word?", "What do you think happens between layer 1 and the last layer that makes the answer so much better?"],
        simplification: "The flashing questions ('Does a cat have fur?', 'What rhymes with cat?') are an analogy. Each layer doesn't literally ask a question — it performs mathematical transformations. But research (particularly 'probing' studies) has shown that different types of information do emerge at different depths. The questions we show are inspired by what researchers have found at each layer range, presented in kid-friendly language.",
      },
      {
        keyPoints: ["Many layers — each pass makes understanding richer", "Early layers = spelling, late layers = reasoning"],
        narrative: "The takeaway. Ask: 'If you cut the model off at layer 32, what could it do? What couldn't it do?' The early-to-late progression (spelling → grammar → facts → meaning) is the thing to remember.",
        definitions: [],
